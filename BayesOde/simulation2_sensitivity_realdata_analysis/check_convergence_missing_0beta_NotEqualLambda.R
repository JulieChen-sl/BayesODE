...
check_convergence_0beta_NotEqualLambda<-function(yobsv_missing,n.par,delta_t,n.track)
{
  ...
  for(k_mc in 1:4)
  {
    ...
    yobsv<-add_data_lambda(yobsv_missing,theta=theta_initial,k1=1,k2=2,n.track)
    c1<-NULL;c2<-NULL
  ...
  repeat
  {
    x.val<-mcmcsample_beta0(yobsv_missing,n.par,delta_t,n.track,n,n0,
                      mc1,mc2,mc3,mc4,sd_pro)
    chains<-NULL
    chains[[1]]<-as.mcmc(x.val[[1]])[,-2]
    chains[[2]]<-as.mcmc(x.val[[2]])[,-2]
    chains[[3]]<-as.mcmc(x.val[[3]])[,-2]
    chains[[4]]<-as.mcmc(x.val[[4]])[,-2]
    ...
    if(MPSRF<1.1||(n>8e+06))
    {
      ...
      x.mean<-x[1:4]
      yobsv_ex<-reset_yobsv_lambda(x,yobsv_missing)
      f_esti_ex<-f_lambda(x.mean,yobsv_ex,delta_t,n.track)
      ...
      #Computing IC
      max_value<-maxlikeli(steady,yobsv_missing)
